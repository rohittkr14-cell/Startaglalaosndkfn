...
# ======================= CONFIG =======================
API_ID = 37893084                 # from my.telegram.org
API_HASH = "853a6c0f3be11009f667bc153244452e"       # from my.telegram.org
BOT_TOKEN = "8642917919:AAHR1U_FtsL_X9XtUROTvpXc6lgjNLDz83M"     # from @BotFather
...
DB_FILE = "settings.db"                    # SQLite database file
...
def get_setting(chat_id: int) -> bool:
    conn = sqlite3.connect(DB_FILE)
    row = conn.execute(
        "SELECT only_admins FROM settings WHERE chat_id = ?", (str(chat_id),)
    ).fetchone()
    conn.close()
...
def set_setting(chat_id: int, only_admins: bool):
    conn = sqlite3.connect(DB_FILE)
    conn.execute(
        "INSERT OR REPLACE INTO settings (chat_id, only_admins) VALUES (?, ?)",
...
def get_all_groups():
    """Return list of (chat_id, title) for all stored groups."""
    conn = sqlite3.connect(DB_FILE)
...
async def all_trigger(client, message: Message):
    ...
    # Text after the trigger keyword (if any)
    ...
    if extra:
        # Tag once with the user's text — no loop
        await tag_once(chat_id, members, extra)
    else:
        # Continuous loop, just mentions — until /stopall
...
async def broadcast_cmd(client, message: Message):
    ...
    # Get broadcast text: from command args, or from the replied message
    ...
    if not groups:
        await message.reply_text(
            ...
            "Bot stores groups automatically when added. "
            "Re-add it to groups if needed."
        )
        return
...
async def clone_cmd(client, message: Message):
    await message.reply_text(
        ...
        "4. Update `API_ID`, `API_HASH`, `BOT_TOKEN`, `BOT_USERNAME`, `ADMIN_IDS` in bot.py\n"
        "5. `pip install pyrogram tgcrypto`\n"
        "6. `python bot.py`\n\n"
...
async def main():
    ...
    # Start the Pyrogram client manually
    await app.start()
    ...
    # Backfill: store all groups the bot is currently in (works via MTProto dialogs)
    try:
        count = 0
        ...
        print(f"[!] Dialog backfill failed (groups added after this will still be tracked): {e}")
    ...
    # Stop the client properly when shutting down
    await app.stop()
